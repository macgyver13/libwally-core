...
capsule_dtor(wally_musig_keyagg_cache, wally_musig_keyagg_cache_free)
capsule_dtor(wally_musig_secnonce, wally_musig_secnonce_free)
capsule_dtor(wally_musig_pubnonce, wally_musig_pubnonce_free)
capsule_dtor(wally_musig_aggnonce, wally_musig_aggnonce_free)
capsule_dtor(wally_musig_session, wally_musig_session_free)
capsule_dtor(wally_musig_partial_sig, wally_musig_partial_sig_free)
...
%pybuffer_nullable_binary(const unsigned char* msg32, size_t msg32_len);
...
%pybuffer_nullable_binary(const unsigned char* priv_keys, size_t priv_keys_len);
...
%pybuffer_nullable_binary(const unsigned char* tweak, size_t tweak_len);
...
%pybuffer_nullable_binary(const unsigned char* xpubs, size_t xpubs_len);
%pybuffer_nullable_binary(void* bytes, size_t bytes_len);
%pybuffer_output_binary(unsigned char* abf_out, size_t abf_out_len);
%pybuffer_output_binary(unsigned char* agg_pk_out, size_t agg_pk_out_len);
...
#ifndef BUILD_STANDARD_SECP
%py_opaque_struct(wally_musig_keyagg_cache);
%py_opaque_struct(wally_musig_secnonce);
%py_opaque_struct(wally_musig_pubnonce);
%py_opaque_struct(wally_musig_aggnonce);
%py_opaque_struct(wally_musig_session);
%py_opaque_struct(wally_musig_partial_sig);

/* MuSig2 functions return their opaque types via output parameters that
 * are not named "output" (cache_out, secnonce_out/pubnonce_out,
 * aggnonce_out, session_out, partial_sig_out, child_out). The
 * %py_opaque_struct in-typemap only matches the literal name "output",
 * so without the typemaps below the raw pointer-to-pointer argument
 * leaks into the Python signature and passing None crashes in the
 * argout typemap. These mirror the Java typemaps for the same names. */
%typemap(in, numinputs=0) struct wally_musig_keyagg_cache **cache_out
                          (struct wally_musig_keyagg_cache *w) { w = 0; $1 = ($1_ltype)&w; }
%typemap(in, numinputs=0) struct wally_musig_aggnonce **aggnonce_out
                          (struct wally_musig_aggnonce *w)     { w = 0; $1 = ($1_ltype)&w; }
%typemap(in, numinputs=0) struct wally_musig_partial_sig **partial_sig_out
                          (struct wally_musig_partial_sig *w)  { w = 0; $1 = ($1_ltype)&w; }
%typemap(in, numinputs=0) struct wally_musig_session **session_out
                          (struct wally_musig_session *w)      { w = 0; $1 = ($1_ltype)&w; }
%typemap(in, numinputs=0) struct wally_musig_secnonce **secnonce_out
                          (struct wally_musig_secnonce *w)     { w = 0; $1 = ($1_ltype)&w; }
%typemap(in, numinputs=0) struct ext_key **child_out
                          (struct ext_key *w)                  { w = 0; $1 = ($1_ltype)&w; }

/* nonce_gen/nonce_gen_counter return a (secnonce, pubnonce) pair; wrap
 * the two consecutive opaque outputs as a Python 2-tuple of capsules. */
%typemap(in, numinputs=0) (struct wally_musig_secnonce **secnonce_out,
...
%typemap(argout) (struct wally_musig_secnonce **secnonce_out,
    ...
    if (*$1 != NULL && *$2 != NULL) {
        ...
        PyTuple_SetItem(tuple, 0, PyCapsule_New(*$1, "struct wally_musig_secnonce *", destroy_wally_musig_secnonce));
        PyTuple_SetItem(tuple, 1, PyCapsule_New(*$2, "struct wally_musig_pubnonce *", destroy_wally_musig_pubnonce));
        Py_DecRef($result);
        $result = tuple;
    }
}
