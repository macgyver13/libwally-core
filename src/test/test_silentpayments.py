...
class SilentPaymentsTests(unittest.TestCase):
    ...
    def test_resolve_clears_modifiable_flags(self):
        """Test storing a missing output script fixes the transaction"""
        case = BIP352_JSON[0]['sending'][0]['given']
        keys, keys_len = make_cbuffer(''.join(v['private_key'] for v in case['vin']))
        entropy, entropy_len = make_cbuffer(ENTROPY)
        TXMOD_SINGLE = 0x04  # WALLY_PSBT_TXMOD_SINGLE
        mod_all = 0x01 | 0x02 | TXMOD_SINGLE  # Inputs and Outputs Modifiable too

        # Resolving clears Inputs and Outputs Modifiable, but leaves the
        # SIGHASH_SINGLE flag, which records a signature rather than mutability
        psbt = self.build_psbt(case['vin'], case['recipients'])
        self.assertEqual(wally_psbt_set_tx_modifiable_flags(psbt, mod_all), WALLY_OK)
        self.assertEqual(wally_psbt_sp_resolve(psbt, keys, keys_len,
                                               entropy, entropy_len, 0), WALLY_OK)
        self.assertEqual(psbt.contents.tx_modifiable_flags, TXMOD_SINGLE)
        script, script_len = make_cbuffer('00' * 34)
        ret, _ = wally_psbt_get_output_script(psbt, 0, script, script_len)
        self.assertEqual(ret, WALLY_OK)
        wally_psbt_free(psbt)

        # With every script proposed, nothing is set and the flags are kept
        psbt = self.build_psbt(case['vin'], case['recipients'])
        self.assertEqual(wally_psbt_set_output_script(psbt, 0, script, script_len), WALLY_OK)
        self.assertEqual(wally_psbt_set_tx_modifiable_flags(psbt, mod_all), WALLY_OK)
        self.assertEqual(wally_psbt_sp_resolve(psbt, keys, keys_len,
                                               entropy, entropy_len, 0), WALLY_OK)
        self.assertEqual(psbt.contents.tx_modifiable_flags, mod_all)
        wally_psbt_free(psbt)

        # The collaborative path clears them when the shares resolve
        _, given, keys = next(self.two_signer_cases())
        psbt = self.build_psbt(given['vin'], given['recipients'])
        self.assertEqual(wally_psbt_set_tx_modifiable_flags(psbt, mod_all), WALLY_OK)
        self.assertEqual(self.contribute(psbt, [0], keys[0]), WALLY_OK)
        self.assertEqual(self.contribute(psbt, [1], keys[1], '11' * 32), WALLY_OK)
        self.assertEqual(psbt.contents.tx_modifiable_flags, mod_all)
        self.assertEqual(wally_psbt_sp_resolve_shares(psbt, 0), WALLY_OK)
        self.assertEqual(psbt.contents.tx_modifiable_flags, TXMOD_SINGLE)
        wally_psbt_free(psbt)
