...
%apply(char *STRING, size_t LENGTH) { (const unsigned char* msg32, size_t msg32_len) };
...
%apply(char *STRING, size_t LENGTH) { (const unsigned char* xpubs, size_t xpubs_len) };
%apply(char *STRING, size_t LENGTH) { (unsigned char* abf_out, size_t abf_out_len) };
%apply(char *STRING, size_t LENGTH) { (unsigned char* agg_pk_out, size_t agg_pk_out_len) };
...
/* END AUTOGENERATED */
%ignore wally_map_keypath_get_bip32_public_key_from;
...
%java_opaque_struct(wally_musig_keyagg_cache, 10);
%java_opaque_struct(wally_musig_secnonce, 11);
%java_opaque_struct(wally_musig_pubnonce, 12);
%java_opaque_struct(wally_musig_aggnonce, 13);
%java_opaque_struct(wally_musig_session, 14);
%java_opaque_struct(wally_musig_partial_sig, 15);
...
%returns_size_t(wally_descriptor_get_musig_num_participants);
%returns_string(wally_descriptor_get_musig_participant_key);
%returns_size_t(wally_descriptor_get_musig_participant_key_features);
%returns_array_(wally_descriptor_get_musig_participant_key_origin_fingerprint, 4, 5, BIP32_KEY_FINGERPRINT_LEN);
%returns_size_t(wally_descriptor_get_musig_participant_key_origin_path_str_len);
...
#ifndef BUILD_STANDARD_SECP
/* The musig/psbt headers use parameter names other than "output" for their
 * opaque-output pointers (cache_out, aggnonce_out, partial_sig_out,
 * session_out, secnonce_out, child_out). The %java_opaque_struct typemap
 * only matches the literal name "output", so without the typemaps below
 * SWIG falls back to a default that takes a `long` raw pointer-to-pointer
 * from the Java caller — which crashes when dereferenced. Each typemap
 * here suppresses the input slot, allocates a local, and feeds &local to
 * the C call. The matching (argout) from %java_opaque_struct fires on
 * type alone and wraps the result. */
%typemap(in, numinputs=0) struct wally_musig_keyagg_cache **cache_out
                          (struct wally_musig_keyagg_cache *w) { w = 0; $1 = &w; }
%typemap(in, numinputs=0) struct wally_musig_aggnonce **aggnonce_out
                          (struct wally_musig_aggnonce *w)     { w = 0; $1 = &w; }
%typemap(in, numinputs=0) struct wally_musig_partial_sig **partial_sig_out
                          (struct wally_musig_partial_sig *w)  { w = 0; $1 = &w; }
%typemap(in, numinputs=0) struct wally_musig_session **session_out
                          (struct wally_musig_session *w)      { w = 0; $1 = &w; }
%typemap(in, numinputs=0) struct wally_musig_secnonce **secnonce_out
                          (struct wally_musig_secnonce *w)     { w = 0; $1 = &w; }
%typemap(in, numinputs=0) struct ext_key **child_out
                          (struct ext_key *w)                   { w = 0; $1 = &w; }

/* MuSig2 nonce_gen / nonce_gen_counter return TWO opaque outputs at once
 * (secnonce + pubnonce). The multi-arg typemap below matches the two
 * parameters appearing consecutively (nonce_gen and nonce_gen_counter
 * are the only two functions in the API where that pair appears, so the
 * typemap is correctly scoped). The Java return type is Object[] of
 * length 2: index 0 = secnonce holder, index 1 = pubnonce holder. */
%typemap(in, numinputs=0) (struct wally_musig_secnonce **secnonce_out,
                           struct wally_musig_pubnonce **pubnonce_out)
                          (struct wally_musig_secnonce *sn_local,
                           struct wally_musig_pubnonce *pn_local) {
    sn_local = 0; pn_local = 0;
    $1 = &sn_local;
    $2 = &pn_local;
}
%typemap(argout) (struct wally_musig_secnonce **secnonce_out,
                  struct wally_musig_pubnonce **pubnonce_out) {
    jclass obj_cls = (*jenv)->FindClass(jenv, "java/lang/Object");
    jobjectArray arr = (*jenv)->NewObjectArray(jenv, 2, obj_cls, NULL);
    if (*$1)
        (*jenv)->SetObjectArrayElement(jenv, arr, 0, create_obj(jenv, *$1, 11));
    if (*$2)
        (*jenv)->SetObjectArrayElement(jenv, arr, 1, create_obj(jenv, *$2, 12));
    $result = arr;
}
%return_decls(wally_musig_nonce_gen,         Object[], jobjectArray)
%return_decls(wally_musig_nonce_gen_counter, Object[], jobjectArray)

/* MuSig2 single-output opaque-returning functions */
%returns_struct(wally_musig_aggnonce_parse,         wally_musig_aggnonce);
%returns_struct(wally_musig_keyagg_cache_parse,     wally_musig_keyagg_cache);
%returns_struct(wally_musig_pubnonce_parse,         wally_musig_pubnonce);
%returns_struct(wally_musig_session_parse,          wally_musig_session);
%returns_struct(wally_musig_partial_sig_parse,      wally_musig_partial_sig);
%returns_struct(wally_musig_pubkey_agg,             wally_musig_keyagg_cache);
%returns_struct(wally_musig_pubkeys_agg_then_derive, ext_key);
%returns_struct(wally_musig_pubkeys_derive_then_agg, wally_musig_keyagg_cache);
%returns_struct(wally_musig_pubkey_to_xpub,         ext_key);
...
%returns_void__(wally_musig_secnonce_free);
%returns_void__(wally_musig_pubnonce_free);
%returns_void__(wally_musig_aggnonce_free);
%returns_void__(wally_musig_session_free);
%returns_void__(wally_musig_partial_sig_free);

/* MuSig2 opaque type serializers */
%returns_array_(wally_musig_keyagg_cache_serialize, 2, 3, WALLY_MUSIG_KEYAGG_CACHE_LEN);
%returns_array_(wally_musig_pubnonce_serialize,     2, 3, WALLY_MUSIG_PUBNONCE_LEN);
%returns_array_(wally_musig_aggnonce_serialize,     2, 3, WALLY_MUSIG_AGGNONCE_LEN);
%returns_array_(wally_musig_session_serialize,      2, 3, WALLY_MUSIG_SESSION_LEN);
%returns_array_(wally_musig_partial_sig_serialize,  2, 3, WALLY_MUSIG_PARTIAL_SIG_LEN);

/* MuSig2 key/signature accessors */
%returns_array_(wally_musig_pubkey_get,              2, 3, EC_PUBLIC_KEY_LEN);
%returns_array_(wally_musig_pubkey_ec_tweak_add,     4, 5, EC_PUBLIC_KEY_LEN);
