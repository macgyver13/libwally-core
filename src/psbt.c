                           ...
                           struct wally_psbt_input *result)
{
    ...
    while (ret == WALLY_OK && (key_len = pull_varlength(cursor, max)) != 0) {
        ...
        if (is_pset_ft) {
            ...
            is_known = field_type < PSBT_FT_LIMIT;
            if (is_known) {
                field_bit = PSBT_FT(field_type);
                is_known = (field_bit & PSBT_IN_SUPPORTED) &&
                           (!is_pset || !(field_bit & PSBT_IN_SP_FIELDS));
                has_keydata = !!(field_bit & PSBT_IN_HAVE_KEYDATA);
            } else if (field_type == PSBT_IN_SP_TWEAK && !is_pset) {
                            ...
                            struct wally_psbt_output *result)
{
    ...
    while (ret == WALLY_OK && (key_len = pull_varlength(cursor, max)) != 0) {
        ...
        if (is_pset_ft) {
            ...
            if (is_known) {
                field_bit = PSBT_FT(field_type);
                is_known = (field_bit & PSBT_OUT_SUPPORTED) &&
                           (!is_pset || !(field_bit & PSBT_OUT_SP_FIELDS));
            }
        }
