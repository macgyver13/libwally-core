 ...
 * Storing one fixes the transaction, as BIP-375 requires of a Signer that
 * sets a missing output script.
 */
    ...
    for (i = 0; i < num_recipients; ++i) {
        ...
        if (existing_len) {
            ...
            ret = wally_psbt_set_output_script(psbt, recipients[i].output_index,
                                               script, script_len);
            /* BIP-375: a Signer that sets a missing script fixes the tx */
            psbt->tx_modifiable_flags &= ~(WALLY_PSBT_TXMOD_INPUTS |
                                           WALLY_PSBT_TXMOD_OUTPUTS);
        }
        if (ret != WALLY_OK)
    ...
    size_t *status_out)
{
    ...
    if (ret == WALLY_OK && status == WALLY_SP_COMPLETE) {
         ...
         * on this clone, which clears the flags. */
