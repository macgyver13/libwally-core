 ...
 *
 * :param psbt: The PSBT to set the share on.
 ...
 * :param share: The compressed ECDH share point.
 * :param share_len: Length of ``share`` in bytes. Must be `EC_PUBLIC_KEY_LEN`.
 *
 * A global share covers the sum of every eligible input, so it can only be
 * produced by a signer holding all of their private keys. It must be
 * accompanied by its proof, see `wally_psbt_set_global_sp_dleq_proof`.
 */
 ...
 *
 * :param psbt: The PSBT to set the proof on.
 * :param scan_key: The recipient's compressed scan public key.
 * :param scan_key_len: Length of ``scan_key`` in bytes. Must be `EC_PUBLIC_KEY_LEN`.
 * :param proof: The DLEQ proof of the corresponding global ECDH share.
 * :param proof_len: Length of ``proof`` in bytes. Must be 64.
 *
 * BIP-375 requires every share to be accompanied by its proof; a share
 * without one makes the PSBT invalid to sign.
 */
 ...
 * inputs, and so cannot produce a global share covering all of them. It must
 * be accompanied by its proof, see `wally_psbt_input_set_sp_dleq_proof`.
 */
 ...
 * :param scan_key: The recipient's compressed scan public key.
 * :param scan_key_len: Length of ``scan_key`` in bytes. Must be `EC_PUBLIC_KEY_LEN`.
 * :param proof: The DLEQ proof of the corresponding input ECDH share.
 * :param proof_len: Length of ``proof`` in bytes. Must be 64.
 *
 * BIP-375 requires every share to be accompanied by its proof; a share
 * without one makes the PSBT invalid to sign.
 ...
 */
WALLY_CORE_API int wally_psbt_input_get_taproot_leaf_script_size(
    const struct wally_psbt_input *input,
    size_t *written);
 ...
 * :param written: Destination for the count.
 */
WALLY_CORE_API int wally_psbt_input_get_taproot_leaf_signature_size(
    const struct wally_psbt_input *input,
    size_t *written);
 ...
 */
WALLY_CORE_API int wally_psbt_input_get_musig2_pubnonce_size(
    const struct wally_psbt_input *input,
    size_t *written);
 ...
 */
WALLY_CORE_API int wally_psbt_input_get_musig2_partial_sig_size(
    const struct wally_psbt_input *input,
    size_t *written);
 ...
 */
WALLY_CORE_API int wally_psbt_input_populate_taproot_from_descriptor(
    struct wally_psbt *psbt,
    size_t index,
 ...
 * :param flags: Must be zero.
 */
WALLY_CORE_API int wally_psbt_output_populate_taproot_from_descriptor(
    struct wally_psbt *psbt,
    size_t index,
    const struct wally_descriptor *descriptor,
    uint32_t multi_index,
...
/**
 * Add a keypath to a given PSBT output.
 *
 * :param psbt: The PSBT to add the keypath to.
 ...
 * :param child_path_len: The number of items in ``child_path``.
 */
WALLY_CORE_API int wally_psbt_add_output_keypath(
    struct wally_psbt *psbt,
    uint32_t index,
    const unsigned char *pub_key,
    size_t pub_key_len,
 ...
 *
 * For each musig() key expression in the descriptor, this function populates:
 *
 ...
 * - `PSBT_OUT_MUSIG2_PARTICIPANT_PUBKEYS`: for each matching output
 *
 * Only inputs whose witness UTXO scriptPubKey matches the descriptor's
 * scriptPubKey for ``child_num``, and outputs whose script matches it, are
 * populated. Other inputs and outputs, including inputs with no witness
 * UTXO, are left untouched.
 *
 * .. note:: BIP-390 aggregate-then-derive expressions, i.e. a ``musig()``
 *|    key with a trailing derivation path, are rejected with `WALLY_EINVAL`:
 *|    signing them requires BIP-328 tweaks that are not yet implemented.
 *|    Give the derivation paths to the participant keys instead, so that the
 *|    aggregate key is itself the taproot internal key.
 *
 ...
 * PSBT under the composite key ``participant || agg_pubkey``.
 ...
 * more than once.
 *
 * ``pubkey33`` must be present in the input's
 * `PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS` entry for ``agg_pubkey``.
 *
 ...
 * :param agg_pubkey_len: Length of ``agg_pubkey``. Must be ``EC_PUBLIC_KEY_LEN``.
 * :param leaf_hash: Must be NULL: only key-path spends are currently supported
 *|    (script-path MuSig2 signing is not yet implemented).
 * :param leaf_hash_len: Length of ``leaf_hash``. Must be 0.
 * :param keyagg_cache: Optional untweaked keyagg cache of the participant keys,
 *|    from `wally_musig_pubkey_agg` or `wally_musig_pubkeys_derive_then_agg`.
 * :param flags: For future use, pass 0.
 ...
 *
 * ``pubkey33`` must be present in the input's
 * `PSBT_IN_MUSIG2_PARTICIPANT_PUBKEYS` entry for ``agg_pubkey``. The BIP-341
 * taproot output tweak is applied internally to a copy of ``keyagg_cache``
 * during signing.
 *
 ...
 *|    (script-path MuSig2 signing is not yet implemented).
 * :param leaf_hash_len: Length of ``leaf_hash``. Must be 0.
 * :param keyagg_cache: The untweaked keyagg cache of the participant keys,
 *|    from `wally_musig_pubkey_agg` or `wally_musig_pubkeys_derive_then_agg`.
 *|    Do not apply any tweak to it: the BIP-341 taproot tweak is applied
 *|    internally, and a pre-tweaked cache produces invalid signatures.
 * :param flags: For future use, pass 0.
 ...
 *
 * Each partial signature is verified before aggregation; if any fails to
 * verify, an error is returned and the input's pubnonces and partial
 * signatures are left in place.
 *
