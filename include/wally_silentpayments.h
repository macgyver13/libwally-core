 ...
 * share and DLEQ proof is stored per unique recipient scan key. If any
 * missing script is stored, the Inputs Modifiable and Outputs Modifiable
 * flags are cleared, as BIP-375 requires of a Signer that sets one.
 *
 ...
 * the operation fails, and storing a missing one clears the Inputs Modifiable
 * and Outputs Modifiable flags, as they do in `wally_psbt_sp_resolve`.
 *
