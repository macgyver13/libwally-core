...
#define WALLY_MS_IS_MUSIG         0x4000 /** A musig() key aggregate (BIP-390) */
 ...
 *
 * .. note:: A musig() key aggregate occupies a key index but has no single
 *|    string representation; `WALLY_EINVAL` is returned for it. Detect it
 *|    with `WALLY_MS_IS_MUSIG` from `wally_descriptor_get_key_features`,
 *|    then use the `wally_descriptor_get_musig_participant_key` family.
 */
...
/**
 * Get the length of the key origin path string for a participant key in a musig() key aggregate.
 *
 * :param descriptor: The descriptor to get the origin path length from.
 * :param index: The index of the musig() key in the descriptor.
 * :param participant_index: The index of the participant within musig().
 * :param written: Destination for the length of the origin path string,
 *|    excluding the NUL terminator (zero if not present).
 */
WALLY_CORE_API int wally_descriptor_get_musig_participant_key_origin_path_str_len(
    const struct wally_descriptor *descriptor,
    size_t index,
    size_t participant_index,
    size_t *written);
