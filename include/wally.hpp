...
template <class INPUT, class SCAN_KEY, class PROOF>
inline int psbt_input_set_sp_dleq_proof(const INPUT& input, const SCAN_KEY& scan_key, const PROOF& proof) {
    int ret = ::wally_psbt_input_set_sp_dleq_proof(detail::get_p(input), scan_key.data(), scan_key.size(), proof.data(), proof.size());
    return detail::check_ret(__FUNCTION__, ret);
}
...
template <class PSBT, class SCAN_KEY, class PROOF>
inline int psbt_set_global_sp_dleq_proof(const PSBT& psbt, const SCAN_KEY& scan_key, const PROOF& proof) {
    int ret = ::wally_psbt_set_global_sp_dleq_proof(detail::get_p(psbt), scan_key.data(), scan_key.size(), proof.data(), proof.size());
    return detail::check_ret(__FUNCTION__, ret);
}

template <class PSBT, class SCAN_KEY, class SHARE>
inline int psbt_set_global_sp_ecdh_share(const PSBT& psbt, const SCAN_KEY& scan_key, const SHARE& share) {
    int ret = ::wally_psbt_set_global_sp_ecdh_share(detail::get_p(psbt), scan_key.data(), scan_key.size(), share.data(), share.size());
    return detail::check_ret(__FUNCTION__, ret);
}
