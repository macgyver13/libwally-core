...
.. c:function:: int wally_psbt_sp_resolve(struct wally_psbt *psbt, const unsigned char *priv_keys, size_t priv_keys_len, const unsigned char *entropy, size_t entropy_len, uint32_t flags)
   ...
   :param priv_keys: The private keys of the PSBT's eligible inputs, concatenated in input order, one `EC_PRIVATE_KEY_LEN` key per input that `wally_psbt_get_input_sp_eligible` reports as eligible.
